...
model_path = "./T-lite-instruct-0.1"
tokenizer = AutoTokenizer.from_pretrained(model_path)
model = AutoModelForCausalLM.from_pretrained(model_path)
model.half()  # Примените это после загрузки модели


# Функция для обработки текста через модель
def process_text_with_model(text, task):
    prompt = f"Ты языковая модель для обработки текста. В твоём ответе должен быть только обработанный текст и ничего больше. {task}: {text}"
...
print(chunks)
topic = process_text_with_model(chunks[0], "Определи тему этой лекции. В ответе напиши только тему")
print('тема:',topic)
...
for chunk in chunks:
    success = False
    while not success:
        processed_text = process_text_with_model(chunk, f'Перепиши текст, сократив его, оставь только самое главное. Учти, что в тексте есть лишние фразы, не касающиеся основной темы ("{topic}"), их вставлять в текст не нужно.')
