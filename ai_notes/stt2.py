...
# Загрузка модели Whisper для STT (оставлю закомментированным, если используется Whisper)
...
# Инициализация GGUF модели с помощью llama.cpp
model_path = "путь_к_вашей_gguf_модели"
llm_model = llama_cpp.Llama(model_path=model_path)


# Функция для обработки текста через GGUF модель
def process_text_with_llm(text, task):
    prompt = f"{task}: {text}"

    output = llm_model(prompt, max_tokens=256)
...
for chunk in split_text(text):
    print(i)
    i += 1
    corrected_text += process_text_with_llm(chunk, "Исправь ошибки распознавания") + " "
...
# Создание конспекта
summary = ""
for chunk in split_text(corrected_text):
    print(i)
    i += 1
    summary += process_text_with_llm(chunk, "Создай конспект по тексту") + " "
